...
- New first-party Composer plugin `vitamind/archive-plugin` (developed at `dev-packages/vitamind-archive-plugin/`, namespace `VitaminD\Plugins\Archive\`) providing folders and files with four visibility tiers per item: `user`, `workspace`, `app`, `public`.
...
### New Capabilities
- `vitamind-archive-plugin`: folder/file storage plugin — folder and file CRUD, four-tier per-item visibility authorization, secure UUID-based physical storage independent of any web-reachable disk, upload validation (whitelist mime/extension cross-check), and a single authorized download/view endpoint.
