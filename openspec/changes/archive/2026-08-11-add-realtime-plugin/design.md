...
- **Sanctum + Reverb channel-auth interplay**: the `/broadcasting/auth` route's composition with the existing `web` middleware/session stack (and, separately, the Sanctum guard for bearer-token requests) is verified by `ChannelAuthorizationTest` against a real HTTP request — that risk is closed. What remains unverified is the *browser* side: an actual Echo/Reverb client sending its session cookie through that route has not been checked in a real browser session (see below).
- **Migrating already-shipped UI** (`AppHeader`'s WiFi indicator, `Layout`'s bootstrap-invalidation refresh) risks a visible regression if the Echo-based replacement doesn't preserve the exact `{status, reconnect}` contract those components read. Mitigation: D6 keeps that return shape unchanged. **Not yet done**: manual in-browser verification (clicking the WiFi indicator, watching a live `bootstrap.invalidated` refresh) — task 7.3 covers broadcast-timing documentation, not this; the PR's test plan explicitly lists browser click-through as outstanding, relying instead on the automated HTTP-level tests plus a live `reverb:start` + `pusher-js` smoke test that received one real broadcast end-to-end.
...
**Rollback**: the flag defaults to `false`, so disabling `vitamin-d.features.websocket` alone reverts all *runtime* behavior with no data migration involved (no new persisted tables are introduced by this package itself). Fully removing the package as installed code is more than deleting the composer path-repo entry and the package directory — it also means reverting the root `composer.json`/`package.json` entries, the `.env.example` additions, and the migrated `resources/js` files (`socket-store.ts`, `use-socket-events.ts`, `app-header.tsx`, `layout.tsx`) back to their pre-D6 hand-rolled-WebSocket-scaffold form, since that scaffold is replaced, not left in place, by this change.
