...
- [x] 7.7 (new) Deploy the above to `clients-lab`/`wagw.nugrahadi.com` — done 2026-08-07; full runbook + live verification record at `wakuwaku-provisioner/DEPLOYMENT_PLAN.md` (Phases 0-5 all passed against production, including the Caddy listen-address code change). Deployed under the `wakuwaku-provisioner` name (the repo and its deployment were both renamed from `waini-provisioner` the same day; migrated live, not left running under the old name).

## 8. Cross-cutting verification

- [ ] 8.1 (partially done, see note) End-to-end test: add the first WhatsApp number in a fresh workspace through to linked status, against a real or staging `waini-provisioner` + GoWA instance — unblocked by §7.7 and partially done 2026-08-07: `CreateInstance`/`GetInstanceStatus`/`DeleteInstance` exercised end-to-end against production `clients-lab` (via `php artisan tinker`, not a HTTP-layer test — see 8.3 for the transcript), instance reached `active` with a real GoWA process and a working `base_url`. Still open: actual device *linking* (QR/pairing-code scan) needs a human with a phone — not something verifiable by an agent. The API/provisioning layer is confirmed working end-to-end; only the final manual pairing step is unverified.
...
- [x] 8.3 Verify create-instance idempotency and rollback-on-failure behavior against the running provisioner — done 2026-08-07 against production `clients-lab`: created a throwaway local workspace, called `CreateInstance::handle()` twice (second call returned the identical `WaInstance` row — same `base_url`/username, no reallocation), `GetInstanceStatus` → `active`, `DeleteInstance` → local row removed and remote status confirmed `absent` afterward. WakuWaku-side idempotent-caller behavior is also covered by `tests/Feature/ProvisionerClientTest.php` (mocked), and the provisioner's own idempotency/rollback logic has its own passing Go test suite (`internal/provisioner/manager_test.go`) in the other repo; rollback-on-failure specifically (an actual failed create) was not forced/re-verified live here, only exercised via those existing test suites.
