...
`vitamind/core` and `vitamind/workspace-plugin` SHALL NOT be considered stable enough for Packagist publishing until at least 2 of the following 3 consumer projects have successfully extended this boilerplate via a `dev-packages/` path repository, with no breaking change required to either package: BukuWarga, LembarUji, UangKas.

#### Scenario: Fewer than 2 projects have integrated successfully
- **WHEN** fewer than 2 of the 3 named consumer projects have completed integration without requiring a breaking change to `vitamind/core` or `vitamind/workspace-plugin`
...
- **WHEN** at least 2 of the 3 named consumer projects have completed integration (via `composer require vitamind/core`, plus the workspace plugin if used) without requiring a breaking change
