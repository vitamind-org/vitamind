# plugin-identity-declaration Specification

## Purpose

Both VitaminD plugin systems — Composer-package `ServiceProvider`-based plugins (extending `VitaminD\PluginSdk\PluginBase`) and dynamically-installed plugins (extending `VitaminD\PluginSdk\AbstractPlugin`, covering local, GitHub, and Composer sources) — need a single, consistent way to declare their own identity (key, name, description). This capability ensures every plugin declares that identity itself via a required `pluginDetails()` method, and that role registration and installation/display metadata are derived from that declared identity rather than guessed from namespaces or duplicated across separate getter methods.
