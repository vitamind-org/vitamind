# plugin-installation-system Specification

## Purpose

The plugin installation system is the robust mechanism VitaminD uses to install, discover, track, and activate plugins from three sources: Composer packages (1st-party, from Packagist), GitHub repositories (vitamind-org or community), and Local plugins (development/application-specific). It ensures the registry tracks each plugin's source and metadata, validates dependencies before enabling, runs a plugin's pending migrations automatically on enable, and keeps installation atomic and recoverable regardless of source.
