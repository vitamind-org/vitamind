# vitamind-realtime-plugin Specification

## Purpose

Realtime Plugin (`vitamind/realtime-plugin`) is the optional, feature-flagged package that adds WebSocket broadcasting on top of VitaminD Core: a Laravel Reverb broadcaster, a Sanctum-aware channel-authorization route, and frontend Echo wiring that feeds incoming broadcast events into the application's existing TanStack Query cache. It works identically whether or not `vitamind/workspace-plugin` is installed, and additionally exposes a convention-based, workspace-scoped channel-authorization helper when workspaces are active. It does not prescribe channel-naming topology or broadcast-timing strategy — those remain per-event implementor decisions.
