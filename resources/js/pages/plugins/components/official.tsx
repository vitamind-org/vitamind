...
export default function OfficialPlugins({ marketplace }: { marketplace: MarketplaceConfig }) {
  const org = marketplace?.github?.org || 'vitamind-org';
  const topic = marketplace?.github?.topic || 'vitamind-plugin';
  ...
  const query = useInfiniteQuery<{
    ...
    queryKey: ['official-plugins', org, topic],
    queryFn: async ({ pageParam }) => {
      const data = (
        await axios.get(`https://api.github.com/search/repositories?q=owner:${org}%20topic:${topic}&per_page=10&page=${pageParam}`)
      ).data;
